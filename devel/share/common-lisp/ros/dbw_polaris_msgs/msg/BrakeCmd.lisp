...
(cl:defmethod cl:initialize-instance :after ((m <BrakeCmd>) cl:&rest args)
  ...
  (cl:unless (cl:typep m 'BrakeCmd)
    (roslisp-msg-protocol:msg-deprecation-warning "using old message class name dbw_polaris_msgs-msg:<BrakeCmd> is deprecated: use dbw_polaris_msgs-msg:BrakeCmd instead.")))
...
(cl:defmethod pedal_cmd_type-val ((m <BrakeCmd>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader dbw_polaris_msgs-msg:pedal_cmd_type-val is deprecated.  Use dbw_polaris_msgs-msg:pedal_cmd_type instead.")
  (pedal_cmd_type m))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<BrakeCmd>)))
  ...
  (cl:format cl:nil "# Brake pedal~%# Options defined below~%float32 pedal_cmd~%uint8 pedal_cmd_type~%~%# Enable~%bool enable~%~%# Clear driver overrides~%bool clear~%~%# Ignore driver overrides~%bool ignore~%~%# Watchdog counter (optional)~%uint8 count~%~%uint8 CMD_NONE=0~%uint8 CMD_PERCENT=2   # Percent of maximum torque, range 0 to 1~%uint8 CMD_TORQUE=3    # Nm, range 0 to 8000, open-loop~%uint8 CMD_TORQUE_RQ=4 # Nm, range 0 to 8000, closed-loop~%~%float32 TORQUE_MAX=8000 # Nm, maximum torque~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'BrakeCmd)))
  "Returns full string definition for message of type 'BrakeCmd"
  (cl:format cl:nil "# Brake pedal~%# Options defined below~%float32 pedal_cmd~%uint8 pedal_cmd_type~%~%# Enable~%bool enable~%~%# Clear driver overrides~%bool clear~%~%# Ignore driver overrides~%bool ignore~%~%# Watchdog counter (optional)~%uint8 count~%~%uint8 CMD_NONE=0~%uint8 CMD_PERCENT=2   # Percent of maximum torque, range 0 to 1~%uint8 CMD_TORQUE=3    # Nm, range 0 to 8000, open-loop~%uint8 CMD_TORQUE_RQ=4 # Nm, range 0 to 8000, closed-loop~%~%float32 TORQUE_MAX=8000 # Nm, maximum torque~%~%~%"))
